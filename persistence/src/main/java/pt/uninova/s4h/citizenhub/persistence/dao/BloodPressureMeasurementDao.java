...
public interface BloodPressureMeasurementDao {
    ...
    @Query(value = "WITH agg AS(SELECT ((sample.timestamp - :localDate) / 3600000) % 24 AS hour, "
            ...
            + " SELECT systolic AS systolic, diastolic AS diastolic, mean AS mean, hour AS hourOfDay FROM agg GROUP BY hour ")
    @TypeConverters(EpochTypeConverter.class)
    List<HourlyBloodPressurePanel> selectLastDay(LocalDate localDate);
    ...
    @Query(value = "WITH agg AS(SELECT ((sample.timestamp - :from) / 86400000) % :days AS day, "
            ...
            + " SELECT systolic AS systolic, diastolic AS diastolic, mean AS mean, day AS day FROM agg GROUP BY day")
    @TypeConverters(EpochTypeConverter.class)
    List<DailyBloodPressurePanel> selectSeveralDays(LocalDate from, LocalDate to, int days);
